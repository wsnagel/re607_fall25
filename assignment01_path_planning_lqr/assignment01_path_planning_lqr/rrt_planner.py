...
class State(object):
    ...
    def __lt__(self, other):
        """
        Comparitor to break ties for same priority states.
        """
...
class RRTPlanner(object):
    ...
    def __init__(self, world):
        ...
        grid_shape = self.occ_grid.shape
    ...
    def sample_state(self):
        ...
        # x must be in {0, cols-1} and y must be in {0, rows -1} (set of integers).
        x = 0
        y = 0
...
def main(args=None):
    ...
    if len(sys.argv) < 2:
        print("Usage: ros2 run assignment01_path_planning_lqr rrt map.pkl")
        sys.exit(1)

    pkl_file = open(os.path.join('install', package_name, 'share', package_name, 'worlds', sys.argv[1]), 'rb')
    # world is a numpy array with dimensions (rows, cols, 3 color channels)
    world = pickle.load(pkl_file, encoding='latin1')
    pkl_file.close()
