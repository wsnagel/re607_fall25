...
class HuskyMapper(Node):
    def __init__(self, num_rows, num_cols, meters_per_cell):
        ...
        self.declare_parameter('odometry_position_noise_std_dev',0.0)
        self.declare_parameter('odometry_orientation_noise_std_dev',0.0)
        self.odometry_position_noise_std_dev = self.get_parameter('odometry_position_noise_std_dev').value
        self.odometry_orientation_noise_std_dev = self.get_parameter('odometry_orientation_noise_std_dev').value
        
        og_origin_in_map_frame = np.array([-20.0, -10.0, 0.0])
        ...
        self.declare_parameter('max_laser_range',0.0)
        self.declare_parameter('min_laser_range',0.0)
        self.declare_parameter('max_laser_angle',0.0)
        self.declare_parameter('min_laser_angle',0.0)
        self.odometryFirst = False
        ...
        self.declare_parameter('q_map_baselink',rclpy.parameter.Parameter.Type.DOUBLE_ARRAY)   # 4x1 quaternion from husky_1/baselink to map frame
        self.declare_parameter('R_map_baselink',[])   # 3x3 rotation matrix from husky_1/baselink to map frame
        self.declare_parameter('p_map_baselink',rclpy.parameter.Parameter.Type.DOUBLE_ARRAY)   # 3x1 position of husky_1/baselink in map frame
        
        self.declare_parameter('q_map_baselaser',rclpy.parameter.Parameter.Type.DOUBLE_ARRAY)   # 4x1 quaternion from husky_1/baselaser to map frame
        self.declare_parameter('R_map_baselaser',[])   # 3x3 rotation matrix from husky_1/baselaser to map frame
        self.declare_parameter('p_map_baselaser',rclpy.parameter.Parameter.Type.DOUBLE_ARRAY)   # 3x1 position of husky_1/baselaser in map frame
        
        self.declare_parameter('q_baselink_baselaser',rclpy.parameter.Parameter.Type.DOUBLE_ARRAY)
        self.q_baselink_baselaser = np.array([1.0, 0, 0, 0])
        self.declare_parameter('R_baselink_baselaser',[]) 
        self.R_baselink_baselaser = tr.quaternion_matrix(self.q_baselink_baselaser)[0:3,0:3]
        self.declare_parameter('p_baselink_baselaser',rclpy.parameter.Parameter.Type.DOUBLE_ARRAY)
        self.p_baselink_baselaser = np.array([0.337, 0.0, 0.308])

        # Threading to prevent shared variables from being access/written over simultaneously in code
        self.mutex = Lock()


        # Set up publisher and subscribers
        self.occupancy_grid_pub = self.create_publisher(OccupancyGrid, 'husky_1/occupancy_grid', 1)
        self.laser_points_marker_pub = self.create_publisher(Marker, 'husky_1/debug/laser_points', 1)
        self.robot_pose_pub = self.create_publisher(PoseStamped, 'husky_1/debug/robot_pose', 1)

        self.laser_sub = self.create_subscription(
            LaserScan, #Message type
            'husky_1/scan',    #Name of topic
            self.laser_scan_callback,
            1)
        self.odometry_sub = self.create_subscription(
            Odometry, #Message type
            'husky_1/odometry/ground_truth',    #Name of topic
    ...
    def odometry_callback(self, msg):
        ...
        odometry.pose.pose.position.x += random.gauss(0, self.odometry_position_noise_std_dev)
        odometry.pose.pose.position.y += random.gauss(0, self.odometry_position_noise_std_dev)
        ...
        #       these frames are located on the robot, run: ros2 run rviz2 rviz2 and look at the TF
        ...
        #       and map, which is where odometry messages are expressed in. In other words, odometry 
        ...
        # Corrupting the quaternion with noise in yaw.
    ...
    def from_laser_to_map_coordinates(self, points_in_baselaser_frame):
        ...
        # the laser are given with respect to the frame husky_1/base_laser. This function converts
    ...
    def inverse_measurement_model(self, row, col, robot_row, robot_col, robot_theta_in_map, beam_ranges, beam_angles):
        alpha = 0.1
        beta = 10*pi/180.0
        p_occupied = 0.999 #Play around with this number - you might not want to be so confident if you have noisy odometry
        ...
        # TODO: Perform range checks and return appropriate log odds ratio needed for update. Some helpful info:
        # Maximum range value is set in self.max_laser_range.
        # r_cb is the range value of the closest-in-heading beam according to your diff_angle calculation.
        # Be sure to perform float calculations when dividing by numbers (e.g., x/2.0 rather than x/2).
        # You've imported the log function from numpy, so you can directly call it here.
        # Note: This whole function will be called to calculate a change in log odds ratio!
    ...
    def laser_scan_callback(self, msg):
        ...
        if self.odometryFirst is False:
        ...
        for delta_row in range(-max_laser_range_in_cells, max_laser_range_in_cells):
            for delta_col in range(-max_laser_range_in_cells, max_laser_range_in_cells):
                ...
                if self.is_in_field_of_view(baselaser_row, baselaser_col, yaw_map_baselaser, row, col):
...
def main(args=None):
    ...
    num_cols = 250
    meters_per_cell = 0.2
    node = HuskyMapper(num_rows, num_cols, meters_per_cell)
    rclpy.spin(node)
    node.destroy_node()
    rclpy.shutdown()
