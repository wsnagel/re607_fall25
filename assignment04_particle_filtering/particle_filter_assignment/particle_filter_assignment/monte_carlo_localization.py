...
class ParticleFilter(object):
    ...
    def get_random_free_state(self):
        while True:
            ...
            xrand = np.random.uniform(self.xmin*1.0, self.xmax*1.0)
            yrand = np.random.uniform(self.ymin*1.0, self.ymax*1.0)
...
class MonteCarloLocalization(Node):
    def __init__(self, num_particles, xmin, xmax, ymin, ymax):
        ...
        self.declare_parameter('dynamics_translation_noise_std_dev', 0.0)
        dynamics_translation_noise_std_dev   = self.get_parameter('dynamics_translation_noise_std_dev').value
        
        self.declare_parameter('dynamics_orientation_noise_std_dev', 0.0)
        dynamics_orientation_noise_std_dev   = self.get_parameter('dynamics_orientation_noise_std_dev').value
        
        self.declare_parameter('beam_range_measurement_noise_std_dev', 0.0)
        beam_range_measurement_noise_std_dev = self.get_parameter('beam_range_measurement_noise_std_dev').value
        ...
        self.particles_pub = self.create_publisher(MarkerArray, '/husky_1/particle_filter/particles', 1)
        self.odom_sub = self.create_subscription(
            Odometry,
            '/husky_1/odometry/ground_truth',  
    ...
    def laser_scan_callback(self, msg):        
        ...
        if self.last_scan:
            current_ns = msg.header.stamp.sec * 1_000_000_000 + msg.header.stamp.nanosec
            previous_ns = self.last_scan.header.stamp.sec*1_000_000_000 + self.last_scan.header.stamp.nanosec
            dt_since_last_scan = (current_ns - previous_ns)/1_000_000_000.0 
    ...
    def publish_laser_pts(self, msg):
        ...
        lpmarker = self.get_2d_laser_points_marker(msg.header.stamp, 'map', pts_in_map, 30000, [1.0, 0.0, 0, 1.0])
    ...
    def get_particle_marker(self, timestamp, particle, marker_id):
        ...
        #msg.lifetime = rclpy.duration.Duration(seconds=1)
        ...
        point2 = Point()
        point2.x = particle.x + 0.3*vx
        point2.y = particle.y + 0.3*vy
        point2.z = 0.2
